...
namespace usbguard
{
  ...
  AuditEvent Audit::policyEvent(const AuditIdentity& identity, std::shared_ptr<Rule> rule, Policy::EventType event_type)
  {
    ...
    event.setKey("ns", "Policy");
    event.setKey("op", Policy::eventTypeToString(event_type));
    event.setKey("rule_id", numberToString(rule->getRuleID()));
  ...
  AuditEvent Audit::policyEvent(const AuditIdentity& identity, std::shared_ptr<Rule> new_rule, std::shared_ptr<Rule> old_rule)
  {
    ...
    event.setKey("ns", "Policy");
    event.setKey("op", Policy::eventTypeToString(Policy::EventType::Update));
    event.setKey("rule_id", numberToString(old_rule->getRuleID()));
    event.setKey("rule_old", old_rule->toString());
    event.setKey("rule_new", new_rule->toString());
  ...
  AuditEvent Audit::policyEvent(const AuditIdentity& identity, std::shared_ptr<Device> device, Policy::EventType event_type)
  {
    ...
    event.setKey("ns", "Policy.Device");
    event.setKey("op", Policy::eventTypeToString(event_type));
    event.setKey("target", Rule::targetToString(device->getTarget()));
    event.setKey("device", device->getDeviceRule()->toString());
  ...
  AuditEvent Audit::policyEvent(const AuditIdentity& identity, std::shared_ptr<Device> device, Rule::Target old_target, Rule::Target new_target)
  {
    ...
    event.setKey("ns", "Policy.Device");
    event.setKey("op", Policy::eventTypeToString(Policy::EventType::Update));
    event.setKey("target_old", Rule::targetToString(old_target));
    event.setKey("target_new", Rule::targetToString(new_target));
    event.setKey("device", device->getDeviceRule()->toString());
  ...
  AuditEvent Audit::deviceEvent(const AuditIdentity& identity, std::shared_ptr<Device> device, DeviceManager::EventType event_type)
  {
    ...
    event.setKey("ns", "Device");
    event.setKey("op", DeviceManager::eventTypeToString(event_type));
    event.setKey("device", device->getDeviceRule()->toString());
  ...
  AuditEvent Audit::deviceEvent(const AuditIdentity& identity, std::shared_ptr<Device> new_device, std::shared_ptr<Device> old_device)
  {
    ...
    event.setKey("ns", "Device");
    event.setKey("op", DeviceManager::eventTypeToString(DeviceManager::EventType::Update));
    event.setKey("device_old", old_device->getDeviceRule()->toString());
    event.setKey("device_new", new_device->getDeviceRule()->toString());
